...
def main():
    ...
    # sur l'eclaircissement adouci si rien n'a pu etre mesure.
    if os.environ.get('FABMESH_TRELLIS2_SKIP_BRIGHTEN') != '1':
        try:
            accorde = (os.environ.get('FABMESH_TEX_ACCORD', '1') == '1'
                       and _accorder_couleurs_source(glb, img, log=log))
        except Exception as _e:
            log(f'accord des couleurs ignore: {type(_e).__name__}: {_e}')
            accorde = False
        if not accorde:
    ...
    try:
        _n = _strip_opaque_alpha(glb)
        if _n:
            log(f'alpha retire de {_n} atlas (materiau OPAQUE)')
    except Exception as _e:
        log(f'retrait alpha ignore: {_e}')
